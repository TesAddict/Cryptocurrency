...
__global__
void sha256ComputeH0(uint32_t *block_header)
{
	uint32_t w[64];
...
__global__
void sha256Init(uint32_t *block_header, uint32_t threads, uint32_t *h1, uint32_t *h2)
{
	sha256ComputeH2(block_header, threads, h1, h2);
}
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		if (h0_found == false)
		{
			sha256ComputeH0<<<1,1>>>(block_header);
			h0_found = true; 
		}

		sha256Init<<<blocks, threads_per_block>>>(block_header, nonce_append, h1, h2);

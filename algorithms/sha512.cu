...
__device__
void sha512Compute(uint64_t *valid_in,int idx,uint64_t nonce,int dif,uint8_t *byte)
{
	uint64_t in[8];
	uint64_t out[8];
...
__global__
void sha512Init(uint64_t *in, uint64_t nonce, int dif, uint8_t *byte)
{
	int idx = blockIdx.x*blockDim.x + threadIdx.x;	
	sha512Compute(in,idx,nonce,dif,byte);
}
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		sha512Init<<<block_count,thread_count>>>(in,nonce,dif,byte);
	...
	printf("%ld hash attempts performed in %f seconds at %d difficulty.\n", counter,cpu_time_used,dif);

...
struct state_sha512 state_array[TOTAL_STATES];
...
void blake512_compress(struct state_sha512 *S, const uint8_t *block )
{
...
void blake512_init(struct state_sha512 *S)
{
...
void blake512_update(struct state_sha512 *S, const uint8_t *in, uint64_t inlen)
{
  int left = S->buflen;
  int fill = 128 - left;
...
void blake512_final(struct state_sha512 *S, uint8_t *out)
{
...
{
  blake512_init(state_array);
  blake512_update(state_array, in, inlen);
  blake512_final(state_array, out);
}
